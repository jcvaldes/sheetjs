...
program
	...
	.option('--locale <lcid>', 'use specified locale when reading file')
	.option('--read-dpi <dpi>', 'use specified DPI when reading file')
	.option('--write-dpi <dpi>', 'use specified DPI when reading file')
	...
	.option('--template', 'blindly pass certain options')
...
if(program.template) opts.template = true;
if(program.readDpi) opts.PPI = +program.readDpi || 96;
if(program.writeDpi) wopts.PPI = +program.writeDpi || 96;
if(program.locale) X.SSF.setlocale(program.locale || "en-US");

if(program.callback) opts.callback = function(cbinfo) {
	if(program.read) return;
	if(cbinfo.row.length == 0) return;
	console.log(X.utils.cell_array_to_csv_row(cbinfo.row, {RS:""}));
};
...
if(program.read || program.callback) process.exit(0);
